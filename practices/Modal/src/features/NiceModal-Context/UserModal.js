...
import { createNiceModal, NiceModal, NiceModalContext } from "./NiceModal/NiceModal"
...
const UserModal =  createNiceModal('user-modal',({title,user,...props})=>{ 
    ...
    const modal = useContext(NiceModalContext);
    const userModal = useMemo(()=>(modal['user-modal']),[modal])
...
const UserModalWithContext = ()=>(<ContextRenderer>
    {({userModal})=>(<UserModal {...{
            ...userModal
            }} />)}
</ContextRenderer>)

export default UserModalWithContext;
